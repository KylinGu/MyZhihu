...
public class BottomBar extends FrameLayout implements View.OnClickListener, View.OnLongClickListener {
    ...
    //++, kylin, to enable unselect all, always show title and disable scale features.
    private boolean mEnableUnselectAll = false;
    private boolean mEnableAlwasyShowTitle = false;
    private boolean mDisableScaleTitle = false;

    /**
     * If you want to unselect all the tab at initialization stage, then you can set
     * this value to true. In default, it will select the first one.
     *
     * @author kylin
     * @param enable
     */
    public void setEnableUnselectAll(boolean enable){
        mEnableUnselectAll = enable;
    }

    /**
     * If you want to enable always show title, then you must set this value to true.
     * In default, it will NOT show title if it is not selected.
     * However, if it is table mode, it will NOT be effective.
     *
     * @author kylin
     * @param enable
     */
    public boolean setEnableAlwasyShowTitle(boolean enable){
        return mEnableAlwasyShowTitle = mIsTabletMode ? false:enable;
    }

    /**
     * If you do not want to scale tab when it is selected, then you can set this value to true.
     * Defaultly, it will scale to 0.86f.
     *
     * @author kylin
     * @param disable
     */
    public void setDisableScaleTitle(boolean disable){
        mDisableScaleTitle = disable;
    }
    ...
    private void updateItems(BottomBarItemBase[] bottomBarItems) {
        ...
        for (BottomBarItemBase bottomBarItemBase : bottomBarItems) {
            ...
            //m, kylin, to enable always show
            if (mIsShiftingMode && !mIsTabletMode && mEnableAlwasyShowTitle) {
    ...
    private void selectTab(View tab, boolean animate) {
        //++, to enable unselect all.
        if(mEnableUnselectAll && tab == null) return;
    ...
    private void unselectTab(View tab, boolean animate) {
        ...
        //m-s, kylin, to show TextView all the time regardless shiftingmode
        float scale = 0.86f;
        if (!mEnableAlwasyShowTitle){
            scale = mIsShiftingMode ? 0 : 0.86f;
        }else if (mDisableScaleTitle){
            scale = 1;
        }
        //m-e, kylin, to show TextView all the time regardless shiftingmode
